...
int MAX_NUMBER_OF_SHAKING = -1;
int MAX_NUMBER_OF_CALLING_COST_FUNCTION = -1;
int TabuListSize = 20;
int NumberofReachableFacilities;
...
//*************************************MAIN ALGORITHM ********************************************
//************************************************************************************************
int TabuList[MAX_NUMBER_OF_COSTUMER];
vector<int> Tabu(){

	NumberofReachableFacilities=0;
	for(int var=0 ;var<activeFacilityN ; ++var)
		if( minPathCost[HUB_NUMBER][ROOT][var] <INF)
			NumberofReachableFacilities++;

	TabuListSize=min(NumberofReachableFacilities/2,25);
	cout<<"TabuListSize : " <<TabuListSize<<endl;
	...
	for(int var = 0 ;var<activeFacilityN ; ++var)
			current.push_back( (int)(randf()+0.5) );//make random 0/1 
	refine( current );
	bestCost = currentCost = costFunction( current );
	best = current;

	int callofCostFunction=0;
	int bestIterationTime=-1;
	int bestAtRetrunNumber=-1;
	...
	for( it=0;it<NUMBER_OF_ITERATIONS ;++it){
		...
		int bestNeighborCost = INF;
		vector<int>	bestNeghibor;

		vector<int>	neighbor=current;
		int neighborCost;
		int tabuVartemp=-1;

		for(int var=1 ;var<activeFacilityN ; ++var){
			
			if( minPathCost[HUB_NUMBER][ROOT][var] >=INF)
				continue;
			if( TabuList[var]>0 && it < TabuList[var]+TabuListSize )
				continue;
			
			neighbor[var] = 1-neighbor[var]; //change 0-1
			neighborCost = costFunction(neighbor);
			callofCostFunction++;

			if( bestNeighborCost>neighborCost ){
				bestNeghibor=neighbor;
		...
		if(bestNeighborCost < currentCost){
			current=bestNeghibor;
			currentCost=bestNeighborCost;
			TabuList[tabuVartemp]=it;
			if(currentCost<bestCost){
				bestCost=currentCost;
				best=current;
				bestIterationTime=it;
				bestAtRetrunNumber=numberofReturn;
			}
		}else{ //shaking !!!
			numberofReturn++;
			current=bestNeghibor;
			...
			/*
			for(int var = 1 ;var<activeFacilityN ; ++var)
				current[var] = (int)(randf()+0.5);//make random 0/1 
			refine( current );
			currentCost = costFunction( current );
			callofCostFunction++;
			numberofReturn++;
			if(currentCost<bestCost){
				bestCost=currentCost;
	...
	cout<<"Best Iteration at : "<<bestIterationTime<<endl;
	cout<<"Best at Return  : "<<bestAtRetrunNumber<<endl;

	cout<<"Number of Return : "<<numberofReturn<<endl;
	cout<<"Number of costfuntionCall : "<<callofCostFunction<<endl;
	//get the best value ... 
	cout<<bestCost<<endl;
	cout<<costFunction(best)<<endl;
	return best;
}
...
int main(){
	srand(time(NULL));	
	freopen("Tabu CFversion1 1C200-300 it500 .txt","w",stdout);
